<template>
  <div class="app-container">
    ...
    <div class="qq-group">
      <img src="@/assets/fpbrowser-qq-group.png" />
      <p>
        QQ Group:
        <code>564142956</code>
      </p>
          ...
          <el-col :span="12">
            <el-form-item label="查询渠道">
              <el-select v-model="Channel" placeholder="请选择">
                <el-option label="fpbrowser" value="fpbrowser" />
                <el-option label="ipgeoLocation" value="ipgeolocation" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <div v-if="Channel === 'fpbrowser'">
              点击
              <a href="https://fpbrowser.cc" target="_blank" style="color: #42b983">官网</a>
              获取API Key
            </div>
          </el-col>
        </el-row>
...
export default {
  ...
  async created() {
    ...
    loadScript(
      `http://api.fpbrowser.cc/update/check_update.js?c=${
        (this.list || []).length
      }&v=${ver}&t=${Date.now()}`
    )
  },
  ...
  methods: {
    ...
    onExport() {
      ...
      var fileName = 'fp-Browser_' + currentDate + '.json'
    ...
    async saveSettings() {
      ...
      if (this.Channel === 'fpbrowser' && !this.apiLink.includes('fpbrowser')) {
        this.$notify({
          title: '错误',
